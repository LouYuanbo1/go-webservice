...
func (e *Executor) Create[T any, PT PointerModel[T]](ctx context.Context, model PT, opts ...ConflictOption) error {
	...
	if len(opts) > 0 {
		clauseConflict, err := e.clauseOnConflictBuilder(opts...)
		if err != nil {
			return errorx.New(
...
func (e *Executor) FindByIDs[T any, PT PointerModel[T], ID comparable](ctx context.Context, dest *[]T, ids []ID, opts ...OrderOption) error {
	...
	if len(opts) > 0 {
		clauseOrder := e.clauseOrderBuilder(opts...)
...
func (e *Executor) Transaction(ctx context.Context, fn func(ctx context.Context, tx *Executor) error) error {
	err := e.gdb.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		return fn(ctx, NewExecutor(tx))
	})
	if err != nil {
		return errorx.New(
