...
type PrimaryQueryFn[T any, ID comparable, PT gen.PointerModel[T, ID]] func(ctx context.Context, conn gen.Session[T, ID, PT], primaryKey ID, val PT) error
...
type CachedConn[T any, ID comparable, PT gen.PointerModel[T, ID]] interface {
	...
	Query(ctx context.Context, key string, val PT, query QueryFn[T, ID, PT], opts ...gormc.TTLOption) error
	QueryIndex(ctx context.Context, key string, val PT, keyer func(primary ID) string, indexQuery QueryFn[T, ID, PT], primaryQuery PrimaryQueryFn[T, ID, PT], opts ...gormc.TTLOption) error
}
...
func (cc *cachedConn[T, ID, PT]) Query(ctx context.Context, key string, val PT, query QueryFn[T, ID, PT], opts ...gormc.TTLOption) error {
...
func (cc *cachedConn[T, ID, PT]) QueryIndex(ctx context.Context, key string, val PT, keyer func(primary ID) string, indexQuery QueryFn[T, ID, PT], primaryQuery PrimaryQueryFn[T, ID, PT], opts ...gormc.TTLOption) error {
	var primaryKey any
	//查询设置对应的主键
	...
	//查询主键对应的值
	return cc.cache.Take(ctx, keyer(primaryKey.(ID)), val, func(val any) error {
		return primaryQuery(ctx, cc.conn, primaryKey.(ID), val.(PT))
	}, gormc.TTLBuilder(cc.cfg.TTL, opts...).GetTTL())
}
