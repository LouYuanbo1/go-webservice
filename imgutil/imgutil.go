...
type ImgUtil interface {
	...
	Thumbnail(img image.Image, opts ...TransformOption) image.Image
	Save(img image.Image, filename string, opts ...SaveOption) error
...
func (i *imgUtil) Thumbnail(img image.Image, opts ...TransformOption) image.Image {
...
// 保存图片,按照配置的质量保存
func (i *imgUtil) Save(img image.Image, filename string, opts ...SaveOption) error {
	...
	switch ext {
	case ".jpg", ".jpeg":
		err = imaging.Save(img, fullPath, imaging.JPEGQuality(save.quality))
	case ".png":
